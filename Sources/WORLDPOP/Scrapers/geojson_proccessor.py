...
def simplify_coordinates(geometry, tolerance=0.01):
    ...
    """
    if geometry['type'] == ('MultiPolygon','Polygon'):
...
def process_geojson(input_data, output_dir= os.path.join(os.getcwd,"Sources/WORLDPOP/data/Scaper_data/Geojson_shapefiles")):
    """
    ...
    Args:
        ...
        output_dir (str, optional): _description_. Defaults to "subdistricts_geojson".
    """
    ...
    for feature in data['features']:
        ...
        sub_district_feature = deepcopy(feature)

        # Convert MultiPolygon to Polygon
        sub_district_feature['geometry'] = convert_multipolygon_to_polygon(
            sub_district_feature)

        # Simplify coordinates
        sub_district_feature['geometry'] = simplify_coordinates(
            sub_district_feature['geometry'])

        # Create district-wise GeoJSON
        district_geojson = {
            "type": "FeatureCollection",
            "features": [sub_district_feature]
        }
...
def main():
    try:
        process_geojson(glob.glob(os.path.join(os.getcwd(),"Maps/Geojson/*_subdistricts.geojson")))
